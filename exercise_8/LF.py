...
def gradient_search_angular_dist(Rd):
    N = Rd.shape[0]
    R = Rd[0,:,:]
    r = np.ones(3)
...
def procrustes_minimazation_chordal(Rd):
    H = np.zeros((3,3));
...
if __name__ == '__main__':
    ...
    Rsvd = procrustes_minimazation_chordal(Rd)
    R_grad_search = gradient_search_angular_dist(Rd)
